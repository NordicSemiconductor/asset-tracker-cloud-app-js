...
import { CredentialsConsumer, IotConsumer } from '../App'
							...
							<CatLoader catId={catId} describeThing={describeThing}>
								{(cat, update) => (
									<Cat
										...
										getThingState={async () => getThingState(iotData)(catId)}
										listenForStateChange={async ({ onNewState, onMessage }) =>
										...
										onAvatarChange={avatar => {
											...
											avatarUploader(avatar)
												.then(async url => attributeUpdater({ avatar: url }))
												.catch(console.error)
										}}
										onNameChange={name => {
