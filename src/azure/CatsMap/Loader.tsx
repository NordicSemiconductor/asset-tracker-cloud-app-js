...
export const CatMapLoader = ({ apiClient }: { apiClient: ApiClient }) => {
	...
	useEffect(() => {
		...
		apiClient
			...
			.then((res) => {
				if (isRight(res)) {
					const q = `SELECT c.deviceId, c.deviceUpdate.properties.reported.gps.v FROM c WHERE c.deviceId IN (${(res
						.right.result as { deviceId: string }[]).map(
						(s) => `"${s.deviceId}"`,
					)}) AND c.deviceUpdate.properties.reported.gps != null AND c.timestamp IN (${(res
						.right.result as { max_timestamp: string }[]).map(
						(s) => `"${s.max_timestamp}"`,
					)})`
